...
Carousel.prototype._element = function(){
	...
	if (this.default.direction === "verticality") {	//垂直方向
		...
		//判断是否溢出
		if(this.el.parentNode.clientWidth < this.el.parentNode.scrollWidth){
			//给当前元素一定宽度
...
Carousel.prototype._setInterval = function(){
	if(this.isOverflowing){ 
        ...
        this.Interval = setInterval(function(){
            this.top --;     
            if(Math.abs(this.top) >= this.firstWH){   //当移动距离>=内容高度，就重新计算 
                this.top = 0;
            }
            //给当前元素添加样式
